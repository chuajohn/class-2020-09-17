...
clean_names()%>%
select(u_grad, course_name,course_title)%>%
  filter(u_grad >= 10)%>%
  filter(u_grad < 20000)%>%
arrange(desc(u_grad))
raw

#Principles of Economics has the most number of undergrads enrolled, with 559 students.
#Gov 50 has 110 students.
...
ggplot(data=raw, mapping=aes(x=course_title, y=u_grad))+
  geom_col()
